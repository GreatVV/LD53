namespace Fusion.KCC
{
	...
	public sealed unsafe class KCCNetworkQuaternion<TContext> : KCCNetworkProperty<TContext> where TContext : class
	{
		...
		public KCCNetworkQuaternion(TContext context, float accuracy, Action<TContext, Quaternion> set, Func<TContext, Quaternion> get, Func<TContext, float, Quaternion, Quaternion, Quaternion> interpolate) : base(context, 4)
		{
		...
		public override void Interpolate(InterpolationData interpolationData)
		{
			...
			{
				value = Quaternion.Lerp(fromValue, toValue, interpolationData.Alpha);
			}
